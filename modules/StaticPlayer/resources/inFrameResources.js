...
var mw = (function($, undefined) {
	...
	return {
		...
		legacy: {},
		messages: new Map(),
		message: function(key, parameter_1) {
			var parameters;
		...
		loader: (function() {
			...
			function addInlineCSS(css) {
				...
				if ($style.is('style') && $style.data('ResourceLoaderDynamicStyleTag') === true) {
					...
					$newStyle = $(addStyleTag(css, getMarker())).data('ResourceLoaderDynamicStyleTag', true);
				}
			}
			...
			return {
				...
				using: function(dependencies, ready, error) {
					...
					if (compare(filter(['ready'], dependencies), dependencies)) {
						...
						if ($.isFunction(error)) {
							error(new Error('one or more dependencies have state "error"'), dependencies);
						}
					} else {
...
(function(mw, $) {
	...
	$(mw).bind('InterfacesReady', function() {
		mw.interfacesReadyFlag = true;
	});
	$(document).ready(function() {
...
/* cache key: resourceloader:filter:minify-js:7:a2b150524104f0cb9ecb62dec5b675c5 */